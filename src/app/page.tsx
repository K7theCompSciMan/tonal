...
import OfflinePlaylistControls from "@/components/OfflinePlaylistControls";
import { fetchLyrics } from "@/lib/lyrics";
import {
	filterLocalLibrary,
	searchAllStreamingSources,
} from "@/lib/externalMusicSearch";
import type { ParsedTrack } from "@/lib/playlist-parser";
import type { ResolvedTrack } from "@/lib/search";
import { toYouTubeEmbedUrl } from "@/lib/youtube";
import { saveQueueState, loadQueueState } from "@/lib/offlineStorage";
import {
	type PlayableTrack,
	getStreamUrl,
	getCoverUrl,
...
import { setMediaSessionMetadata, clearMediaSession } from "@/lib/trackUtils";
...
function getTrackKey(track: PlayableTrack, fallback: string | number): string {
	return isDbSong(track)
		? track.id
		: `${track.title}-${track.artist}-${getStreamUrl(track)}-${fallback}`;
}
...
function ShuffleIcon() {
	return (
		<svg
			...
			aria-hidden="true">
			<path
				d="M4 7h2.4c2.1 0 3.4 1.1 4.7 3.1l1.8 2.8c1.2 1.9 2.4 3.1 4.7 3.1H20"
				stroke="currentColor"
				strokeWidth="2"
				strokeLinecap="round"
			/>
			...
			/>
			<path
				d="M4 17h2.4c1.5 0 2.5-.5 3.4-1.5"
				stroke="currentColor"
				strokeWidth="2"
				strokeLinecap="round"
			/>
			<path
				d="M14.2 8.5c.9-1 1.9-1.5 3.4-1.5H20"
				stroke="currentColor"
				strokeWidth="2"
				strokeLinecap="round"
			/>
	...
	return (
		<div
			...
			<button type="button" onClick={onPlay} className="tonal-track-main">
				<span className="tonal-track-index">
					{isCurrent ? "▶" : index + 1}
				</span>
...
export default function TonalApp() {
	...
	const [queueOrderBeforeShuffle, setQueueOrderBeforeShuffle] = useState<
		PlayableTrack[] | null
	>(null);
	...
	useEffect(() => {
		saveQueueState(queue, currentIndex).catch(() => {});
	}, [queue, currentIndex]);
	useEffect(() => {
		loadQueueState().then((saved) => {
	...
	useEffect(() => {
		...
		preloadAudio(getStreamUrl(nextTrack));
	}, [queue, currentIndex]);
	...
	const fetchData = async (userId: string | null = user?.id ?? null) => {
		try {
			...
			const playlistQuery = supabase
				...
				.select("*")
				.order("created_at", { ascending: false });
			const { data: pls } = userId
				? await playlistQuery.eq("owner_id", userId)
				: { data: [] };
	...
	useEffect(() => {
		...
		void withTimeout(supabase.auth.getSession(), SESSION_TIMEOUT_MS)
			...
			.catch((error) => {
				...
				console.error(error);
				setAuthMessage(
					"Could not reach Supabase. You can still try logging in.",
				);
			})
			.finally(() => {
	...
	const handleAuthSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			console.error(error);
			setAuthMessage(
				"Supabase did not respond. Check your project URL, anon key, and network.",
			);
		} finally {
	...
	const handlePlaylistImport = async (
		...
		if (!user) {
			window.alert(
				"Sign in to Supabase (authenticated session) to import playlists.",
			);
			return;
		}
		...
		for (const t of tracks) {
			...
			const streamUrl = isYt
				? (toYouTubeEmbedUrl(r.streamUrl) ?? r.streamUrl)
				: r.streamUrl;
	...
	const saveExternalTrackToLibrary = async (track: PlayableTrack) => {
		...
		const streamUrl = isYt
			? (toYouTubeEmbedUrl(track.streamUrl) ?? track.streamUrl)
			: track.streamUrl;
	...
	const addTrackToPlaylist = async (
		...
		try {
			...
			if (exists) {
				setStatusMessage(
					`"${dbSong.title}" is already in ${playlist.name}.`,
				);
				return;
			}
	...
	const removeTrackFromPlaylist = async (
		...
		const nextSongs = selectedPlaylistSongs.filter(
	...
	const editStreamLink = async (track: PlayableTrack) => {
		if (!isDbSong(track)) {
			setStatusMessage(
				"Save this web result to your library before editing its link.",
			);
			return;
		}
		if (!user) {
		...
		const sourceType =
			nextUrl.includes("youtube.com") || nextUrl.includes("youtu.be")
				? "youtube"
				: "direct";
		const streamUrl =
			sourceType === "youtube"
				? (toYouTubeEmbedUrl(nextUrl) ?? nextUrl)
				: nextUrl;
		const { data, error } = await supabase
			.from("songs")
		...
		if (error || !data) {
			console.error(error);
			setStatusMessage(
				"Could not update this stream link. You may not own this song row.",
			);
			return;
		}
		...
		setQueue((items) =>
			items.map((item) =>
				isDbSong(item) && item.id === updated.id ? updated : item,
			),
		);
	...
	const renameSelectedPlaylist = async () => {
		...
		const name = window
			.prompt("Playlist name", selectedPlaylist.name)
			?.trim();
	...
	const deleteSelectedPlaylist = async () => {
		...
		setPlaylists((items) =>
			items.filter((playlist) => playlist.id !== selectedPlaylist.id),
		);
		setSelectedPlaylistId(null);
		setSelectedPlaylistSongs([]);
	};
	...
	const renderAuthPanel = (compact = false) => (
		<section
			...
			{authLoading ? (
				<p className="tonal-muted text-sm text-gray-400">
					Checking session...
				</p>
			) : user ? (
				...
				<form
					...
					className="tonal-auth-form space-y-3">
					<div>
						<p className="text-sm font-bold text-white">
							{authMode === "sign-in"
								? "Log in to Tonal"
								: "Create your Tonal account"}
						</p>
						<p className="tonal-muted mt-1 text-xs text-gray-400">
							Save playlists, imports, and library changes with
							your account.
						</p>
					</div>
					<input
					...
					{authMessage && (
						<p className="tonal-error text-xs text-[#f15e6c]">
							{authMessage}
						</p>
					)}
				</form>
	...
	return (
		<div className="tonal-shell flex h-dvh bg-black text-white overflow-hidden font-sans pb-[env(safe-area-inset-bottom)]">
			...
			<aside className="tonal-sidebar hidden md:flex w-[240px] bg-black p-2 flex-col gap-2 shrink-0">
				...
				{renderAuthPanel()}
				<div className="h-[96px] md:h-24"></div>
			</aside>

			{/* MAIN */}
			<main className="tonal-main flex-1 m-0 md:m-2 rounded-none md:rounded-lg overflow-y-auto p-4 pt-[calc(4.25rem+env(safe-area-inset-top))] md:p-6 pb-44 md:pb-28 bg-[linear-gradient(to_bottom,#222222,#121212)]">
				...
				{activeTab === "home" && (
					<div>
						<div className="tonal-page-head mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
							<div>
								<h1 className="tonal-title text-2xl md:text-3xl font-bold">
									Welcome back
								</h1>
								<p className="tonal-copy mt-1 text-sm text-gray-400">
									{user
										? "Your Supabase session is active."
										: "Log in to save playlists and imports."}
								</p>
							</div>
							{!user && (
								<button
						...
						<div className="tonal-grid grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 md:gap-4">
							{librarySongs.map((song) => (
								<button
									...
									<p className="tonal-track-title font-bold truncate text-sm md:text-base">
										{song.title}
									</p>
									<p className="tonal-track-meta text-sm text-gray-400 truncate">
										{song.artist}
									</p>
								</button>
							))}
				...
				{activeTab === "search" && (
					<div>
						...
						<p className="tonal-muted text-xs text-gray-500 mb-6">
							Searching your library, YouTube, Jamendo, and iTunes
						...
						<div className="tonal-results space-y-1 max-w-3xl">
							{searchSections.map((section) => (
								<section
									key={section.label}
									className="tonal-search-section">
									<h2 className="tonal-section-heading">
										{section.label}
									</h2>
				...
				{activeTab === "library" && (
					<div>
						{selectedPlaylist ? (
							<section>
								...
								<div className="tonal-playlist-hero">
									...
									<div className="tonal-playlist-copy">
										<p className="tonal-muted text-xs font-bold uppercase">
											Playlist
										</p>
										<h1 className="tonal-playlist-title">
											{selectedPlaylist.name}
										</h1>
										{selectedPlaylist.description && (
											<p className="tonal-copy">
												{selectedPlaylist.description}
											</p>
										)}
										<p className="tonal-muted text-sm">
											{selectedPlaylistSongs.length} song
											{selectedPlaylistSongs.length === 1
												? ""
												: "s"}
										</p>
									</div>
								...
								<div className="tonal-playlist-actions">
									...
									<OfflinePlaylistControls
										playlistName={selectedPlaylist.name}
										tracks={selectedPlaylistSongs}
										onImport={(tracks) => {
											setQueue(tracks);
											setCurrentIndex(0);
										}}
								...
								{playlistLoading ? (
									...
									<div className="tonal-empty">
										...
										<p className="tonal-muted text-sm">
											Use song menus or swipe search
											results to add tracks.
										</p>
									</div>
								) : (
									<div className="tonal-track-list">
										<div className="tonal-track-list-head">
											<span>#</span>
											<span>Title</span>
											<span className="tonal-hide-mobile">
												Album
											</span>
											<span>Actions</span>
										</div>
										{selectedPlaylistSongs.map(
											(song, index) => (
												<SwipeableTrackRow
													key={song.id}
													track={song}
							...
							<>
								<h1 className="tonal-title text-2xl font-bold mb-4">
									Your Library
								</h1>
								<div className="tonal-library-head mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
									<p className="tonal-copy text-gray-400 text-sm">
										Open a playlist to view songs, edit
										details, play, or shuffle.
									</p>
									<button
										type="button"
										onClick={() =>
					...
					<div className="tonal-panel-body flex-1 overflow-y-auto p-2">
						{queue.length === 0 ? (
							<p className="text-gray-500 text-sm p-4">
								Play a track to build your queue.
							</p>
						) : (
							queue.map((track, i) => (
								<button
									...
									{i === currentIndex && (
										<span className="text-[#1DB954] text-xs shrink-0">
											Playing
										</span>
									)}
								</button>
			...
			<footer className="tonal-player fixed bottom-0 left-0 right-0 z-30 h-[96px] md:h-24 bg-black border-t border-[#282828] px-3 md:px-4 flex items-center gap-2 md:gap-4 pb-[max(0.65rem,env(safe-area-inset-bottom))] md:pb-3">
					...
					{currentSong && (
						<button
							...
							<div className="tonal-player-info overflow-hidden hidden min-[430px]:block">
								<p className="tonal-track-title text-sm font-bold truncate">
									{currentSong.title}
								</p>
								<p className="tonal-track-meta text-xs text-gray-400 truncate">
									{currentSong.artist}
								</p>
							</div>
						</button>
				...
				<div className="tonal-controls flex-1 flex flex-col items-center gap-1 md:gap-2 min-w-0">
					<div className="tonal-control-row flex items-center gap-2 min-[430px]:gap-3 md:gap-6">
						...
						<button
							type="button"
							onClick={handlePrev}
							className="tonal-control text-lg md:text-2xl"
							aria-label="Previous">
							<PreviousIcon />
						</button>
						<button
				...
				<div className="tonal-player-tools w-[32%] md:w-[30%] flex justify-end items-center gap-1 md:gap-2 shrink-0">
					...
					<input
						...
						value={volume}
						onChange={(e) =>
							setVolumeLocalStorage(Number(e.target.value))
						}
			...
			<div
				className="fixed pointer-events-none opacity-0 w-px h-px overflow-hidden"
				aria-hidden>
				<ReactPlayer
					ref={playerRef}
					src={playbackUrl || ""}
			...
			{trackMenu && (
				<div
					className="tonal-action-backdrop"
					onClick={() => setTrackMenu(null)}>
					<div
						className="tonal-action-sheet"
						onClick={(e) => e.stopPropagation()}>
						<div className="tonal-action-head">
							<div className="tonal-min">
								<p className="tonal-track-title">
						...
						<div className="tonal-action-group-title">
							Add to playlist
						</div>
						{playlists.length === 0 ? (
							<p className="tonal-muted text-sm">
			...
			{statusMessage && (
				<div className="tonal-toast">{statusMessage}</div>
			)}
